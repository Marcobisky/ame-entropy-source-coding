...
function save_encoded_message(binary_encoded, filename)
    % Save the binary-encoded message in chunks for large data handling
    fileID = fopen(filename, 'w');
    chunk_size = 1024; % Chunk size for saving
    for i = 1:chunk_size:length(binary_encoded)
        chunk = binary_encoded(i:min(i+chunk_size-1, length(binary_encoded)));
        fprintf(fileID, '%s\n', chunk); % Save each chunk as a new line
    end
    fclose(fileID);
end
